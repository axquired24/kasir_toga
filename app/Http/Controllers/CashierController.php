...
class CashierController extends Controller
{
    public function setOption(Request $request)
    {
        ...
        if (isset($request['peringatan_stock'])) {
            ...
            $option->save();
            $notifikasi = ['cls' => 'succcess', 'text' => 'Pengaturan Toko berhasil diupdate'];
    ...
    public function getProduct($namaOrKode)
    {
        ...
        if (isset($product->kode)) {
            $ret = ['tipe' => 'single', 'product' => $product];
        ...
        if (count($product) != 0) {
            $ret = ['tipe' => 'multi', 'product' => $product];
        ...
        // else
        $ret = ['tipe' => 'error', 'product' => 'Produk tidak ditemukan'];
    ...
    public function checkout($option, Request $request)
    {
        ...
        switch ($option) {
            ...
            case 'bayar':
                ...
                if (count($carts) == 0) {
                    $notifikasi = ['cls' => 'warning', 'text' => 'Error. Belanjaan Kosong'];

                    return back()->with('notifikasi', $notifikasi);
                }
                // Cek apakah uang pembayaran Cukup
                if ($dibayar < $total_belanja) {
                    $notifikasi = ['cls' => 'warning', 'text' => 'Error. Uang yang dibayarkan tidak cukup'];
